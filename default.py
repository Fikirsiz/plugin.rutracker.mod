...
CONTENT = {
    ...
    'movie': {
        ...
        'ignore': (
        1640, 1454, 2374, 2373, 185, 254, 771, 44, 906, 69, 267, 65, 772, 789, 941, 1666, 531, 125, 149, 186, 96, 94,
        653, 2344,
    ...
    'music': {
        'index': (1781, 2507, 1812, 2271, 413, 1849, 416, 409, 1125, 408, 1760, 1215,),
        'ignore': (
        1, 1784, 2276, 2511, 466, 465, 1846, 1126, 1127, 1134, 1135, 2018, 855, 473, 782, 2377, 468, 691, 469, 786, 785,
        1631, 1499, 715, 1388, 282, 796, 784, 783, 2331, 2431, 880,
        792, 435, 443, 560, 794, 556, 2307, 557, 2308, 558, 793, 1395, 1396, 436, 2309, 2310, 2311, 969, 1140, 1130,
        1131, 1132, 1133, 2084, 1128, 1129, 1856, 2430, 1283, 2085, 1282, 1284, 1285, 1138, 1136, 1137,
        448, 441, 1173, 1486, 1172, 446, 909, 1665, 1761, 1764, 1766, 1767, 1769, 1765, 1771, 1770, 1768, 1774, 1772,
        1773, 2233, 1218, 1220, 1221, 1334, 1216, 1223, 1224, 1225, 1226),
        'media': 'video',
        'scraper': None,
        'rating': False,
        'stream': True
...
class Scrapers(TrailerParser):
    ...
    def scraper_kinopoisk(self, item):
        ...
        if 'runtime' in kinopoisk['info'] and kinopoisk['info']['runtime']:
            ...
            kinopoisk['info']['plot'] = u''.join(
                [self.lang[40102], u': [B]', kinopoisk['info']['runtime'], u'[/B] ', self.lang[40103], u'\n',
                 kinopoisk['info']['plot']])
    ...
    def scraper_tvdb(self, item):
        ...
        if 'runtime' in tvdb['info'] and tvdb['info']['runtime']:
            if 'plot' not in tvdb['info']:
                tvdb['info']['plot'] = u''
            tvdb['info']['plot'] = u''.join(
                [self.lang[40102], u': [B]', tvdb['info']['runtime'], u'[/B] ', self.lang[40103], u'\n',
                 tvdb['info']['plot']])
...
class MenuRutracker(Handler):
    def handle(self):
        ...
        self.item(Link('rutracker-folder', {'content': 'music'}), title=u'Музыкальные видео')
    ...
    def render_rutracker(self, is_search, folder, data):
        ...
        if err:
            ...
            if not folder and not is_search:
                ...
                items = [x for x in data['data'] if
                         x['type'] == 'torrent' or x['id'] not in CONTENT[self.argv['content']]['ignore']]
            ...
            if not folder and not is_search:
                total += 1
                self.item(Link('rutracker-search', {'content': self.argv['content']}),
                          title=u'[COLOR FF0DA09E][B][' + self.lang[30114] + u'][/B][/COLOR]', total=total)

            # верхний паджинатор
            if data['pages'][1]:
                self.item(Link('rutracker-folder',
                               {'content': self.argv['content'], 'id': self.argv['id'], 'page': data['pages'][1]}),
                          title=u'[COLOR FF0DA09E][B][' + self.lang[30101] + u'][/B][/COLOR] - [' + str(
                              data['pages'][1]) + u'/' + str(data['pages'][0]) + u']',
                          popup=[(Link('setting'), self.lang[40015])], popup_replace=True, total=total)

            for item in items:
                if item['type'] == 'folder':
                    self.item(Link('rutracker-folder', {'content': self.argv['content'], 'id': item['id']}),
                              title=item['name'], popup=[(Link('force-cache',
                                                               {'content': self.argv['content'], 'id': item['id']}),
                                                          self.lang[40030])], total=total)
                else:
                    ...
                    if item['comment'] == -1:
                        ...
                        popup.append((Link('comment', {'id': item['id']}),
                                      self.lang[40004] + u' (' + str(item['comment']) + u')'))

                    # статус раздачи
                    popup.append((Link('status', {'seeder': item['seeder'], 'leecher': item['leecher'],
                                                  'download': item['download'], 'comment': item['comment'],
                                                  'status': item['status'], 'status_human': item['status_human']}),
                                  self.lang[40005] + u' (' + str(item['seeder']) + u'/' + str(item['leecher']) + u')'))

                    # поиск похожих раздач
                    if scraper['search']:
                        popup.append((Link('rutracker-search',
                                           {'content': self.argv['content'], 'search': scraper['search']}, True),
                                      self.lang[40006]))

                    # закладки
                    if scraper['bookmark']:
                        popup.append((Link('bookmark-add',
                                           {'scrapper': scraper['bookmark'][0], 'id': scraper['bookmark'][1]}),
                                      self.lang[40009]))
                    ...
                    if status_view:
                        try:
                            ...
                            scraper['title'] = u'[COLOR ' + STATUS[item['status_human']][1] + ']' + item[
                                'status'] + u'[/COLOR]  ' + scraper['title']
                    ...
                    self.item(Link('download',
                                   {'id': item['id'], 'content': self.argv['content'], 'subdir': scraper['subdir'],
                                    'icon': scraper['icon'], 'thumb': scraper['thumb'], 'fanart': scraper['fanart'],
                                    'bookmark': scraper['bookmark'], 'title': scraper['title']}),
                              title=scraper['title'], icon=scraper['icon'], thumb=scraper['thumb'],
                              fanart=scraper['fanart'], media=CONTENT[self.argv['content']]['media'],
                              info=scraper['info'], popup=popup, popup_replace=True, folder=False, total=total)

            # нижний паджинатор
            if data['pages'][3]:
                self.item(Link('rutracker-folder',
                               {'content': self.argv['content'], 'id': self.argv['id'], 'page': data['pages'][3]}),
                          title=u'[COLOR FF0DA09E][B][' + self.lang[30102] + u'][/B][/COLOR] - [' + str(
                              data['pages'][3]) + u'/' + str(data['pages'][0]) + u']',
                          popup=[(Link('setting'), self.lang[40015])], popup_replace=True, total=total)
    ...
    def render_kinopoisk(self, data):
        if data is None:
            ...
            if data['pages'][1]:
                ...
                self.item(Link(self.link, self.argv),
                          title=u'[COLOR FF0DA09E][B][' + self.lang[30101] + u'][/B][/COLOR] - [' + str(
                              data['pages'][1]) + u'/' + str(data['pages'][0]) + u']',
                          popup=[(Link('setting'), self.lang[40015])], popup_replace=True, total=total)
            ...
            for id in data['data']:
                ...
                if movie:
                    ...
                    # вывод
                    self.item(Link('rutracker-search', {'content': 'movie', 'search': search}),
                              title=movie['info']['title'], thumb=movie['thumb'], media='video', info=movie['info'],
                              fanart=movie['fanart'], popup=popup, popup_replace=True, total=total)
...
class KinopoiskBestQuery(Handler):
    def handle(self):
        ...
        # вывод жанра
        self.item(Link('kinopoisk-best-query', {'change': 'genre'}),
                  title=self.lang[70102] + u': [B]' + genre_lang[genre] + u'[/B]',
                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)

        # вывод даты
        decade_title = u'[B]' + self.lang[70301] + u'[/B]'
        if decade:
            decade_title = u'[B]' + str(decade) + '[/B]-e'
        self.item(Link('kinopoisk-best-query', {'change': 'decade'}), title=self.lang[70103] + u': ' + decade_title,
                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)
        ...
        self.item(Link('kinopoisk-best-query', {'change': 'rate'}), title=self.lang[70104] + u': ' + rate_title,
                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)

        # вывод кол-во оценок
        self.item(Link('kinopoisk-best-query', {'change': 'votes'}),
                  title=self.lang[70105] + u': ' + self.lang[70302] + u' [B]' + str(votes) + u'[/B]',
                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)

        # вывод страны производства
        self.item(Link('kinopoisk-best-query', {'change': 'country'}),
                  title=self.lang[70106] + u':  [B]' + self.kinopoisk.country(country, u' ') + u'[/B]',
                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)

        # вывод MPAA
        self.item(Link('kinopoisk-best-query', {'change': 'mpaa'}),
                  title=u'MPAA: [B]' + (self.lang[70301] if mpaa == 'all' else mpaa) + u'[/B]',
                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)

        # вывод DVD
        self.item(Link('kinopoisk-best-query', {'change': 'dvd'}),
                  title=u'DVD: [B]' + (self.lang[70304] if dvd else self.lang[70303]) + u'[/B]',
                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)

        # кнопка ПОИСК
        self.item(Link('kinopoisk-best',
                       {'genre': genre, 'decade': decade, 'rate': rate, 'votes': votes, 'country': country,
                        'mpaa': mpaa, 'dvd': dvd}),
                  title=u''.join([u'[COLOR FF0DA09E][B][', self.lang[70110], u'][/B][/COLOR]']),
                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)
            ...
            if name:
                ...
                if data is None:
                    ...
                    for d in data['data']:
                        ...
                        self.item(Link('kinopoisk-work', {'id': d['id']}), title=title, thumb=d['poster'],
                                  popup=[(Link('setting'), self.lang[40015])], popup_replace=True)
...
class BookmarkDB:
    ...
    def add(self, scrapper, id):
        ...
        self._connect()
        self.cur.execute('insert into bookmark(addtime,scrapper,id) values(?,?,?)',
        ...
        if not data:
            ...
            for d in data:
                ...
                if d['scrapper'] == 'kinopoisk':
                    ...
                    # удалить из закладок
                    popup.append(
                        (Link('bookmark', {'scrapper': d['scrapper'], 'id': d['id']}), self.lang[40010], True, True))
                    ...
                    # вывод
                    self.item(Link('rutracker-search', {'content': 'series', 'search': search}),
                              title=movie['info']['title'], thumb=movie['thumb'], media='video', info=movie['info'],
                              fanart=movie['fanart'], popup=popup, popup_replace=True, total=total)
    ...
    def handle(self):
        ...
        if stream in ('libtorrent', 'torrentstream', 'delugestream'):
            ...
            if torrent:
                ...
                client = Torrent(client=config['client'], host=config['host'], port=config['port'],
                                 login=config['login'], password=config['password'], url=config['url'])
...
class Stream(TorrentBase):
    ...
    def _delugestream(self):
        ...
        # получаем список файлов из торрента
            ...
            for f in filelist:
                ...
                self.item(Link('stream', self.argv), title=f['name'], media=CONTENT[self.argv['content']]['media'],
                          popup=[(Link('setting'), self.lang[40015])], icon=self.argv['icon'], thumb=self.argv['thumb'],
                          fanart=self.argv['fanart'], popup_replace=True, folder=False, total=total)
    ...
    def _torrentstream(self):
        # проигрываем файл
        if 'file_id' in self.argv:
            torrent = file(xbmc.translatePath('special://temp/plugin.rutracker.torrentstream.cache.torrent'),
                           'rb').read()
            filename = TorrentStream(self.setting['torrentstream_port']).play(torrent, self.argv['file_id'],
                                                                              self.argv['title'], self.argv['icon'],
                                                                              self.argv['thumb'])
            return True
        ...
        # получаем список файлов из торрента
            ...
            filelist = TorrentStream(self.setting['torrentstream_port']).list(torrent, bool(
                self.setting['torrentstream_reverse'] == 'true'))
            if not filelist:
                return True
    ...
    def _libtorrent(self):
        ...
        if 'file_id' in self.argv:
            ...
            filename = LibTorrent().play(torrent=torrent, file_id=self.argv['file_id'], dirname=buffer, seed=seed,
                                         info=None, notice=bool(self.setting['libtorrent_notice'] == 'true'),
                                         buffer=int(self.setting['libtorrent_buffer_size']))
        ...
        # получаем список файлов из торрента
            ...
            # чистим кэш
            if os.path.isfile(self.path('libtorrent', 'buffer.torrent')) and torrent != file(
                    self.path('libtorrent', 'buffer.torrent'), 'rb').read():
                self._clear(buffer)
            ...
            for f in filelist:
                self.argv['file_id'] = f['id']
                self.item(Link('stream', self.argv), title=f['name'], media=CONTENT[self.argv['content']]['media'],
                          info={'size': f['size']}, popup=[(Link('setting'), self.lang[40015])], icon=self.argv['icon'],
                          thumb=self.argv['thumb'], fanart=self.argv['fanart'], popup_replace=True, folder=False,
                          total=total)
    ...
    def _copy(self, filename, dirname):
        ...
        message, fname, fullsize = self.lang[30012], filename.split(os.sep.decode('utf8'))[-1].encode(
            'utf8'), self._human(full).strip()
        ...
        while True:
            ...
            if loop + 0.5 < time.time():
                progress.update(int(load / (full / 100)), message, 'File: ' + fname,
    ...
    def handle(self):
        if xbmcgui.Dialog().yesno('RuTracker', *self.lang[30030].split('|')):
            ...
            while True:
                ...
                progress.update(int(float(page) / (float(total) / 100.0)), self.lang[40801],
                                self.lang[40802] + u':   ' + str(page) + u' / ' + str(total))
            ...
            if items:
                ...
                for id, item in items.iteritems():
                    i += 1
                    progress.update(int(float(i) / (float(total) / 100.0)), self.lang[40803],
                                    self.lang[40804] + u':   ' + str(i) + u' / ' + str(total))
...
class Status(Handler):
    def handle(self):
        line = self.lang[40491] + u':  [B]' + str(self.argv['seeder']) + u'[/B]    ' + self.lang[
            40492] + u':  [B]' + str(self.argv['leecher']) + u'[/B]    ' + self.lang[40493] + u':  [B]' + str(
            self.argv['download']) + u'[/B]'
        ...
        try:
            ...
            xbmcgui.Dialog().ok(self.lang[40005],
                                u'[COLOR ' + color + ']' + self.argv['status'] + u'[/COLOR] ' + self.lang[lang],
                                u'   ' + line)
...
class GuiComment(xbmcgui.WindowXMLDialog):
    ...
    def render(self):
        self.label.setLabel(
            self.lang['comment'] + u':  ' + str(self.cursor + 1) + u'/' + str(len(self.comment)) + u'     ' + self.lang[
                'page'] + u':  ' + str(self.page) + u'/' + str(self.page_total))
        ...
        profile = []
        for tag, lang in (
        ('usertime', self.lang['usertime']), ('count', self.lang['count']), ('location', self.lang['location'])):
            if msg[tag]:
